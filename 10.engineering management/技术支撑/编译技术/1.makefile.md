...
* 核心原理  
Makefile中最核心的内容就是**建立一个文件的依赖关系**。上述语法可简单描述为："target这一个或多个的目标文件依赖于prerequisites中的文件，其生成规则定义在command中"。也就是说，prerequisites中如果有一个或一个以上的文件比target文件的修改日期要新，command所定义的命令就会被执行。
...
```
* 工作原理  
gcc或g++依旧是编译的基本方式：`首先将源码文件(.c)通过gcc -c编译成目标文件(.o), 然后将目标文件(.o)通过gcc -o编译成最终的二进制可执行文件(nty_http_server/.exe)`。而makefile的作用就是组织好各文件的复杂关系，用一个make命令完成gcc或g++的整个过程。而这个复杂关系，正是**从结果（二进制可执行文件）到原因（源代码），通过一级级的依赖，像堆栈一样串行起来的**。如上规则的描述：  
...
像clean这种没有被第一个目标文件直接或间接关联，那么它后面所定义的命令将不会被自动执行。当然，可以显示要make执行，即命令“make clean”，如此清除所有的目标文件，以便重编译。
