...
当自己平时看书、看教程、或编写简单程序，往往发现在一个目录下会创建n个源码文件(如.c文件)，而这里的每一个.c文件都含有一个main函数(即每一个.c文件都生成一个app)。此时每一个.c文件都配置一个makefile，或每一个.c文件都手敲`"gcc -g -o app app.c"`命令，则显得异常繁琐。是否可以写一个makefile,就可以编出n个应用程序呢? 
